...
function renderFilm(moviesList, j) {
  // moviesList.innerHTML = data.map((film) => {
  //   return `
  ...
  //       <p class="movie-card__meta">2025 · ${film.genres.map((genre) => genre.name).join(" | ")} · ${
        ...
        <article class="movie-card">
            <img
              src="	https://image.tmdb.org/t/p/original/${
                data.shows[i].poster_path
              }g"
              alt="How to Train Your Dragon poster"
            />
            <p class="movie-card__title">${data.shows[i].title}</p>
            <p class="movie-card__meta">2025 · ${data.shows[i].genres
              .map((genre) => genre.name)
              .join(" | ")} · ${
      Math.floor(data.shows[i].runtime / 60) +
      "h " +
      (data.shows[i].runtime % 60) +
      "m"
    }</p>
            <div class="movie-card__footer">
              <button class="movie-card__button">Buy Tickets</button>
              <span class="movie-card__rating">
                ...
                ${data.shows[i].vote_average}</span
              >
            </div>
          </article>
    `;
